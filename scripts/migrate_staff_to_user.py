...
try:
    ...
    print("Please run this script from the project root or ensure the PYTHONPATH is set correctly.")
    sys.exit(1)
    ...
    try:
        ...
        for staff in staff_list:
            ...
            if not user:
                ...
                dept_code = staff.department.value[:4].upper() if staff.department else "STAF"
                ...
                print(f"Creating user for {staff.name} (Email: {staff.email}, Mobile: {staff.mobile})")
            ...
            if staff.user_id != user.id:
                ...
                staff_updated += 1
                print(f"Linked staff {staff.name} (ID: {staff.id}) to user (ID: {user.id})")
