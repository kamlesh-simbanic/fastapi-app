...
class TimetableGenerator:
    ...
    def generate_base_data(self):
        ...
        for s, sid in subj_to_id.items():
            self.subject_sql.append(
                f"INSERT INTO subjects (id, name, created_at, updated_at) VALUES ({sid}, '{s}', NOW(), NOW());"
            )
        ...
        for i in range(1, 151):
            ...
            self.teacher_sql.append(
                f"INSERT INTO staff (id, name, mobile, email, department, created_at, updated_at) VALUES ({i}, '{name}', '{mobile}', '{email}', 'TEACHING', NOW(), NOW());"
            )
            ...
            for sname in can_teach:
                ...
                if sid not in self.subject_teacher_map:
                    self.subject_teacher_map[sid] = []
                self.subject_teacher_map[sid].append(i)
                self.teacher_subj_sql.append(
                    f"INSERT INTO teacher_subjects (teacher_id, subject_id, created_at, updated_at) VALUES ({i}, {sid}, NOW(), NOW());"
                )
        ...
        for std in STANDARDS:
            for div in DIVISIONS:
                self.class_data.append(
                    {"id": cid, "standard": str(std), "division": div}
                )
                cid += 1
        ...
        for std in STANDARDS:
            ...
            for div in DIVISIONS:
                class_info = next(
                    ...
                    if c["standard"] == str(std) and c["division"] == div
                )
                ...
                while assigned_count < 36 and max_retries > 0:
                    current_assignments = []
                    {day: {p: set() for p in PERIODS} for day in DAYS}
                    ...
                    for day in DAYS:
                        ...
                        for p in PERIODS:
                            ...
                            for i in range(len(pool)):
                                sname = pool[i]
                                if day_subject_count[day][sname] >= 2:
                                    continue

                                sid = subj_to_id[sname]
                                # Check if any teacher for this subject is free
                                possible_teachers = self.subject_teacher_map.get(
                                    sid, []
                                )
                                ...
                                free_teachers = [
                                    ...
                                    if tid not in self.availability[day][p]
                                ]
                                if free_teachers:
                    ...
                    if possible:
                        ...
                        for a in current_assignments:
                            self.availability[a["day_of_week"]][
                                a["period_number"]
                            ].remove(a["teacher_id"])
                        # Rebuild pool
                        pool = []
                ...
                if max_retries == 0:
                    print(
                        f"Warning: Failed to fully satisfy constraints for Class {std}-{div}"
                    )
    ...
    def write_sql(self, filename):
        with open(filename, "w") as f:
            ...
            for c in self.class_data:
                f.write(
                    f"INSERT INTO school_classes (id, standard, division, created_at, updated_at) VALUES ({c['id']}, '{c['standard']}', '{c['division']}', NOW(), NOW());\n"
                )

            f.write("\n-- Timetable Records\n")
            for i, r in enumerate(self.timetable_records, 1):
                f.write(
                    f"INSERT INTO timetables (id, class_id, subject_id, teacher_id, day_of_week, period_number, created_at, updated_at) VALUES ({i}, {r['class_id']}, {r['subject_id']}, {r['teacher_id']}, '{r['day_of_week']}', {r['period_number']}, NOW(), NOW());\n"
                )
