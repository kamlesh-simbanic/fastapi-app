...
def auto_assign():
    ...
    try:
        ...
        classes = db.query(SchoolClass).order_by(SchoolClass.standard, SchoolClass.division).all()
        ...
        if len(staff_members) < len(classes):
            print(f"Warning: Only {len(staff_members)} staff members available for {len(classes)} classes.")
        ...
        for i, school_class in enumerate(classes):
            if i < len(staff_members):
                ...
                print(f"Assigned {teacher.name} to Standard {school_class.standard} - {school_class.division}")
            else:
                print(f"Warning: No teacher available for Standard {school_class.standard} - {school_class.division}")
