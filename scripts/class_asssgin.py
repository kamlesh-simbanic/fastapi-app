...
from app.models import ClassStudent, SchoolClass, Student
...
def seed_class_assignments():
    ...
    try:
        ...
        db.query(ClassStudent).filter(
            ClassStudent.academic_year == ACADEMIC_YEAR
        ).delete()
        db.commit()
        ...
        for age in range(5, 18):
            ...
            for i, div in enumerate(divisions):
                ...
                if len(assigned_students) < STUDENTS_PER_CLASS:
                    print(
                        f"  Warning: Only {len(assigned_students)} students available for {grade}-{div} (Target: {STUDENTS_PER_CLASS})"
                    )
