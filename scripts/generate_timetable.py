...
for i, s in enumerate(subjects_list, 1):
    subj_to_id[s] = i
    subject_sql.append(
        f"INSERT INTO subjects (id, name, created_at, updated_at) VALUES ({i}, '{s}', NOW(), NOW());"
    )
...
for t in teachers:
    ...
    for s in can_teach:
        ...
        teacher_subj_sql.append(
            f"INSERT INTO teacher_subjects (id, teacher_id, subject_id, created_at, updated_at) VALUES ({ts_id}, {t['id']}, {subj_to_id[s]}, NOW(), NOW());"
        )
        ts_id += 1
        ...
        for std in STANDARDS:
            for div in DIVISIONS:
                ...
                possible_teachers = [
                    ...
                    if ts["subject_id"] == subj_id
                ]
                available_ones = [
                    tid for tid in possible_teachers if tid not in availability[day][p]
                ]
                ...
                if not available_ones:
                    ...
                    all_non_busy = [
                        t["id"] for t in teachers if t["id"] not in availability[day][p]
                    ]
                ...
                timetable_sql.append(
                    f"INSERT INTO timetables (id, class_id, subject_id, teacher_id, day_of_week, period_number, created_at, updated_at) VALUES ({tt_id}, {class_id}, {subj_id}, {teacher_id}, '{day}', {p}, NOW(), NOW());"
                )
                tt_id += 1
