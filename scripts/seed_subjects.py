...
def seed_subjects():
    ...
    try:
        ...
        if len(teachers) < 2:
            print(
                f"Error: Not enough teaching staff found (found {len(teachers)}). Need at least 2."
            )
            return
        ...
        for name in subject_names:
            ...
            if existing_subject:
                print(
                    f"Subject '{name}' already exists. Skipping creation but will check teachers."
                )
                subject = existing_subject
            else:
                subject = Subject(
                    name=name, created_by_id=creator_id, updated_by_id=creator_id
                )
                db.add(subject)
