...
# Read the SQL file
with open('timetable_data.sql', 'r') as f:
    sql = f.read()
...
subject_counts = defaultdict(lambda: defaultdict(int)) # class_id -> subject_id -> count
...
if class_overlaps:
    print(f"FAILED: Found {len(class_overlaps)} class overlaps (multiple teachers in one slot)!")
else:
...
for std_classes in standards:
    ...
    for s in all_subjects:
        if not (counts_a[s] == counts_b[s] == counts_c[s]):
            print(f"FAILED: Frequency mismatch for Standard (Class IDs {std_classes}), Subject {s}: {counts_a[s]}, {counts_b[s]}, {counts_c[s]}")
            frequency_errors += 1
