...
function AttendanceReportContent() {
    ...
    return (
        <div className="space-y-8 animate-in fade-in duration-500 pb-20">
            ...
            <section className="flex flex-col md:flex-row md:items-center justify-between gap-6 font-space">
                <div className="space-y-4">
                    ...
                    <div className="space-y-1">
                        ...
                        <p className="text-muted-foreground text-sm font-medium italic">Detailed attendance analysis and student statistics.</p>
                    </div>
                </div>
                ...
                <div className="flex flex-wrap items-center gap-3">
                    <button
                        ...
                        className="flex items-center gap-2 px-5 py-3 bg-secondary text-zinc-900 dark:text-zinc-100 text-[10px] font-black uppercase tracking-widest rounded-2xl hover:bg-secondary/80 dark:hover:bg-zinc-700 transition-all active:scale-95 disabled:opacity-50"
                    >
            ...
            {/* Selection Controls */}
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6 bg-card p-8 rounded-[2.5rem] border border-border shadow-sm transition-all duration-300">
            ...
            {selectedClass && !loading && report.length > 0 && (
                <div className="grid grid-cols-1 md:grid-cols-4 gap-6 animate-in slide-in-from-bottom-4 duration-500">
                    <div className="bg-card p-6 rounded-[2rem] border border-border shadow-sm">
                        <p className="text-[10px] font-black text-muted-foreground uppercase tracking-widest mb-1">Average Attendance</p>
                        <div className="flex items-end gap-2">
                            <h3 className="text-2xl font-black text-foreground italic">{averageAttendance.toFixed(1)}%</h3>
                            <div className={`mb-1 p-1 rounded-full ${averageAttendance >= 75 ? 'bg-success/10 text-success' : 'bg-amber-500/10 text-amber-500'}`}>
                    ...
                    <div className="bg-card p-6 rounded-[2rem] border border-border shadow-sm">
                        <p className="text-[10px] font-black text-muted-foreground uppercase tracking-widest mb-1">Total Students</p>
                        <h3 className="text-2xl font-black text-foreground italic">{report.length}</h3>
                    </div>

                    <div className="bg-card p-6 rounded-[2rem] border border-border shadow-sm">
                        <p className="text-[10px] font-black text-muted-foreground uppercase tracking-widest mb-1">Class</p>
                    ...
                    <div className="bg-card p-6 rounded-[2rem] border border-border shadow-sm">
                        <p className="text-[10px] font-black text-muted-foreground uppercase tracking-widest mb-1">Academic Year</p>
                        <h3 className="text-2xl font-black text-foreground italic">2023-24</h3>
                    </div>
                </div>
            )}
            ...
            <div className="min-h-[400px]">
                {loading ? (
                    ...
                    <div className="flex flex-col items-center justify-center py-32 gap-6 bg-muted/50 rounded-[3rem] border border-dashed border-border text-center">
                        <div className="w-20 h-20 rounded-full bg-secondary/80 dark:bg-zinc-800 flex items-center justify-center">
                            <Search className="w-10 h-10 text-muted-foreground" />
                        </div>
                        <div>
                            <h3 className="text-xl font-bold text-foreground mb-2 italic">Select class to view report</h3>
                            <p className="text-muted-foreground text-sm max-w-sm mx-auto font-medium">Choose a class, month, and year from the controls above to generate the attendance analysis.</p>
                        </div>
                    </div>
                ) : report.length === 0 ? (
                    <div className="text-center py-32 border-2 border-dashed border-border rounded-[3rem]">
                        <p className="text-muted-foreground font-bold uppercase tracking-widest text-xs">No data for selected period</p>
                    </div>
                ) : (
                    <div className="space-y-6 animate-in slide-in-from-bottom-6 duration-700">
                        <div className="bg-card p-6 rounded-[2.5rem] border border-border shadow-sm flex items-center gap-4">
                        ...
                        <Table
                            ...
                            className="bg-card rounded-[2.5rem] border border-border shadow-xl shadow-zinc-200/50 dark:shadow-none"
                        />
                    </div>
                )}
            </div>
