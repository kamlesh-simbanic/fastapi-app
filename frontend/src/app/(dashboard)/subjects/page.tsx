        ...
        <div className="space-y-8 animate-in fade-in duration-500 pb-10">
            ...
            <section className="flex flex-col md:flex-row md:items-center justify-between gap-6">
                <div className="space-y-1">
                    ...
                    <p className="text-muted-foreground text-sm font-medium">Create and manage academic subjects.</p>
                </div>
            ...
            <div className="flex flex-col lg:flex-row gap-8 min-h-[600px] items-start">
                ...
                <div className={cn(
                    ...
                    {globalLoading.subjects ? (
                        <div className="flex flex-col items-center justify-center py-32 gap-4 text-muted-foreground bg-card rounded-[2.5rem] border border-border shadow-sm">
                        ...
                        <div className="flex flex-col items-center justify-center py-20 gap-4 bg-muted/50 rounded-3xl border border-dashed border-border text-center">
                            ...
                            <p className="text-muted-foreground text-sm max-w-xs">Create subjects to start assigning them to teachers.</p>
                        </div>
                    ) : (
                        <Table
                ...
                <div className="w-full lg:w-1/2 animate-in slide-in-from-right-4 duration-500 sticky top-8">
                    {selectedSubject ? (
                        <div className="bg-card rounded-[2.5rem] border border-border shadow-xl overflow-hidden flex flex-col min-h-[500px]">
                            {/* Panel Header */}
                            <div className="p-8 border-b border-zinc-100 dark:border-border bg-zinc-50/50 dark:bg-zinc-900/50">
                                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                                    <div className="space-y-1">
                                        <div className="px-3 py-1 bg-primary/10 text-primary rounded-full text-[10px] font-black uppercase tracking-widest inline-block mb-1">
                                            Assigned Teachers
                                        </div>
                                        <h2 className="text-2xl font-black text-foreground tracking-tight italic flex items-center gap-3">
                                            {selectedSubject.name}
                                        </h2>
                                    </div>
                                    <button
                                        onClick={() => setIsAssignModalOpen(true)}
                                        className="px-4 py-2 bg-primary text-white rounded-xl text-xs font-bold hover:bg-primary/90 shadow-lg shadow-primary/20 active:scale-95 transition-all flex items-center justify-center gap-2 whitespace-nowrap"
                                    >
                                        <Plus className="w-4 h-4" />
                                        Assign New
                                    </button>
                            ...
                            <div className="flex-1 p-8">
                                {loadingAssignments ? (
                                    ...
                                    <div className="flex flex-col items-center justify-center py-20 gap-4 text-center">
                                        ...
                                        <h3 className="font-bold text-foreground text-lg">No teachers assigned</h3>
                                        <p className="text-muted-foreground text-sm max-w-[240px]">Start by assigning a teacher to this subject.</p>
                                    </div>
                                ) : (
                                    <div className="space-y-4 max-h-[400px] overflow-y-auto pr-2 custom-scrollbar">
                                        {assignments.map((as) => (
                                            <div key={as.id} className="group flex items-center justify-between p-4 rounded-2xl border border-zinc-100 dark:border-border hover:border-primary/20 hover:bg-zinc-50 dark:hover:bg-zinc-800/30 transition-all">
                                                <div className="flex items-center gap-4">
                                                    <div className="w-10 h-10 rounded-xl bg-secondary flex items-center justify-center text-muted-foreground font-bold uppercase">
                                                        {as.teacher?.name.charAt(0)}
                                                    </div>
                                                    <div>
                                                        <h4 className="font-black text-foreground italic tracking-tight">{as.teacher?.name}</h4>
                                                        <p className="text-[10px] font-bold text-muted-foreground uppercase tracking-widest">{as.teacher?.department || 'Teacher'}</p>
                                                    </div>
                                                </div>
                                                <button
                                                    onClick={() => handleUnassign(as.id)}
                                                    className="p-2 text-muted-foreground hover:text-destructive hover:bg-destructive dark:hover:bg-destructive/10 rounded-lg opacity-0 group-hover:opacity-100 transition-all"
                                                    title="Unassign"
                                                >
                            ...
                            {/* Panel Footer */}
                            <div className="p-6 border-t border-zinc-100 dark:border-border bg-zinc-50/30 dark:bg-zinc-900/30">
                                <div className="flex items-center justify-between text-[10px] font-black text-muted-foreground uppercase tracking-widest">
                                    <span>Total Assigned</span>
                                    <span className="bg-white dark:bg-zinc-800 px-3 py-1 rounded-full border border-border dark:border-zinc-700 text-primary">
                        ...
                        <div className="h-full bg-zinc-50/50 dark:bg-zinc-900/30 rounded-[2.5rem] border border-dashed border-border flex flex-col items-center justify-center p-12 text-center text-muted-foreground">
            ...
            {isAddOpen && (
                <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm">
                    <div className="bg-card w-full max-w-md rounded-[2.5rem] border border-border shadow-2xl overflow-hidden">
                        <div className="p-8 border-b border-zinc-100 dark:border-border flex items-center justify-between">
                            <h2 className="text-xl font-black text-foreground">{editingSubject ? 'Edit Subject' : 'Add Subject'}</h2>
                            <button onClick={() => setIsAddOpen(false)} className="p-2 hover:bg-secondary rounded-xl transition-colors"><X className="w-5 h-5" /></button>
                        </div>
                        <form onSubmit={handleSubmit} className="p-8 space-y-6">
                            <div>
                                <label className="text-[10px] font-black text-muted-foreground uppercase tracking-widest mb-2 block">Subject Name</label>
                                <input
                                    required
                                    type="text"
                                    value={formData.name}
                            ...
                            <div className="flex gap-4">
                                <button type="button" onClick={() => setIsAddOpen(false)} className="flex-1 py-3 px-4 rounded-2xl bg-secondary text-muted-foreground dark:text-muted-foreground text-xs font-black uppercase tracking-widest">Cancel</button>
                                <button type="submit" disabled={submitting} className="flex-1 py-3 px-4 rounded-2xl bg-primary text-white text-xs font-black uppercase tracking-widest flex items-center justify-center gap-2">
                ...
                <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm animate-in fade-in duration-300">
                    <div className="bg-card w-full max-w-md rounded-[2.5rem] border border-border shadow-2xl overflow-hidden animate-in zoom-in-95 duration-300">
                        <div className="p-8 border-b border-zinc-100 dark:border-border flex items-center justify-between">
                            ...
                            <button onClick={() => setIsAssignModalOpen(false)} className="p-2 hover:bg-secondary rounded-xl transition-colors font-bold"><X className="w-5 h-5 font-bold" /></button>
                        </div>
                        <div className="p-8 space-y-6">
                            <div>
                                <label className="text-[10px] font-black text-muted-foreground uppercase tracking-widest mb-2 block font-bold">Select Teacher</label>
                                <div className="relative">
                                    <select
                                        value={assigningTeacherId}
                                        onChange={(e) => setAssigningTeacherId(e.target.value)}
                                        className="w-full bg-muted/50 border border-border rounded-2xl py-3.5 px-4 text-sm focus:outline-none focus:ring-4 focus:ring-primary/10 focus:border-primary transition-all font-bold appearance-none cursor-pointer"
                                    >
                            ...
                            <div className="flex gap-4">
                                <button type="button" onClick={() => setIsAssignModalOpen(false)} className="flex-1 py-3 px-4 rounded-2xl bg-secondary text-muted-foreground dark:text-muted-foreground text-xs font-black uppercase tracking-widest font-bold grayscale hover:grayscale-0 transition-all">Cancel</button>
                                <button
                                    onClick={handleAssign}
                                    disabled={submitting || !assigningTeacherId}
                                    className="flex-1 py-3 px-4 rounded-2xl bg-primary text-white text-xs font-black uppercase tracking-widest flex items-center justify-center gap-2 shadow-lg shadow-primary/20 active:scale-95 disabled:grayscale disabled:opacity-50 transition-all font-bold"
                                >
