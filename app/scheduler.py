...
def increment_leave_balance():
    ...
    try:
        ...
        result = db.execute(
            update(Staff).values(leave_balance=Staff.leave_balance + 1)
        )
        db.commit()
        affected_rows = result.rowcount
        logger.info(f"Successfully incremented leave balance for {affected_rows} staff members.")
...
def start_scheduler():
    ...
    scheduler.start()
    logger.info("Scheduler started. Monthly leave increment job scheduled for 12:30 AM on the 1st of every month.")
