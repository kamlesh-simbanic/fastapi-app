...
from .. import schemas, models, controllers
...
def get_current_staff(current_user: models.User = Depends(get_current_user), db: Session = Depends(get_db)):
...
def list_leaves(
    ...
    if view == "approvals":
        return controllers.leave_request.get_leave_requests(db=db, approver_staff=current_staff)
    else:
        return controllers.leave_request.get_leave_requests(db=db, staff_id=current_staff.id)
...
def update_status(
    ...
    return controllers.leave_request.update_leave_status(
        ...
        leave_id=leave_id,
        obj_in=obj_in,
        approver_user_id=current_staff.user_id
    )
