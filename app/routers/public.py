...
PHONEPE_URL = os.getenv(
    "PHONEPE_URL", "https://api-preprod.phonepe.com/apis/hermes/pg/v1/pay"
)
...
def get_public_student_fee(
    gr_no: str, year: int = Query(...), db: Session = Depends(get_db)
):
    student = (
        db.query(models.Student)
        .filter(
    ...
    mappings = (
        ...
        .options(joinedload(models.ClassStudent.school_class))
...
async def initiate_payment(data: dict):
    ...
    payload = {
        ...
        "redirectMode": "POST",
        "callbackUrl": "http://localhost:8000/api/public/pay/callback",
    ...
    try:
        response = requests.post(
            PHONEPE_URL, json={"request": base64_payload}, headers=headers
        )
        ...
        if res_data.get("success"):
            return {
                "success": True,
                "redirectUrl": res_data["data"]["instrumentResponse"]["redirectInfo"][
                    "url"
                ],
            }
        else:
            raise HTTPException(
                status_code=400, detail=res_data.get("message", "Initiation Failed")
            )
