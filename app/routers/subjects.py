...
@router.post("/assign", response_model=schemas.TeacherSubject, status_code=status.HTTP_201_CREATED)
def assign_teacher(
    assignment: schemas.TeacherSubjectCreate,
    db: Session = Depends(get_db),
    current_user: models.User = Depends(get_current_user)
):
    return controllers.subjects.assign_teacher_to_subject(db, assignment, current_user.id)
