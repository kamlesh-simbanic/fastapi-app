...
def create_student(db: Session, student_schema: schemas.StudentCreate):
    ...
    last_student = (
        ...
        .filter(models.Student.gr_no.like(f"{prefix}%"))
    ...
    new_student = models.Student(gr_no=gr_no, **student_schema.model_dump())
    db.add(new_student)
    db.commit()
...
def get_students(
    ...
    items = utils.apply_pagination_sort(
        query, models.Student, skip, limit, sort_by, order
    ).all()
...
def update_student(db: Session, student_id: int, student_update: schemas.StudentUpdate):
    ...
    student_query.update(
        student_update.model_dump(exclude_unset=True), synchronize_session=False
    )
    db.commit()
    db.refresh(student)
    return student
...
def delete_student(db: Session, student_id: int):
    ...
    student_query.update(
        {"status": models.StudentStatus.TERMINATED}, synchronize_session=False
    )
    db.commit()
    return None
