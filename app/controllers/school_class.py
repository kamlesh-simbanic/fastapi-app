...
def add_class(
    db: Session, class_schema: schemas.SchoolClassCreate, current_user_id: int
):
...
def get_class(db: Session, class_id: int):
    school_class = (
        db.query(models.SchoolClass)
        .options(joinedload(models.SchoolClass.class_teacher))
...
def list_classes(
    ...
    query = db.query(models.SchoolClass).options(
        joinedload(models.SchoolClass.class_teacher)
    )
    ...
    items = utils.apply_pagination_sort(
        query, models.SchoolClass, skip, limit, sort_by, order
    ).all()
