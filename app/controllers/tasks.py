...
def get_tasks(
    ...
    return utils.apply_pagination_sort(
        query, models.Task, skip, limit, sort_by, order
    ).all()
...
def get_task(db: Session, task_id: int, current_user_id: int):
    task = (
        ...
        .filter(models.Task.id == task_id, models.Task.created_by == current_user_id)
        .first()
    )
    if not task:
        raise HTTPException(
...
def update_task(
    db: Session, task_id: int, task_update: schemas.TaskUpdate, current_user_id: int
):
    task_query = db.query(models.Task).filter(
        models.Task.id == task_id, models.Task.created_by == current_user_id
    )
