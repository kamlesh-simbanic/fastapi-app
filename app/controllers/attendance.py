...
def add_attendance_bulk_new(
    db: Session, data: schemas.AttendanceBulkCreateNew, current_user_id: int
):
...
def view_attendance(
    ...
    query = db.query(models.Attendance).options(
        joinedload(models.Attendance.school_class)
    )
    ...
    return utils.apply_pagination_sort(
        query, models.Attendance, skip, limit, sort_by, order
    ).all()
...
def view_monthly_attendance_report(db: Session, month: int, year: int, class_id: int):
    ...
    class_mapping = (
        ...
        .filter(models.ClassStudent.class_id == class_id)
        .first()
    )
    ...
    students = (
        ...
        .filter(models.Student.id.in_(class_mapping.students))
        .all()
    )
    student_meta = {
        s.id: {"name": s.name, "surname": s.surname, "gr_no": s.gr_no} for s in students
    }
    ...
    for s_id in report_dict:
        ...
        if total > 0:
            ...
            report_dict[s_id]["attendance_percentage"] = round(
                (present / total) * 100, 1
            )
...
def generate_monthly_attendance_pdf(db: Session, month: int, year: int, class_id: int):
    ...
    school_class = (
        db.query(models.SchoolClass).filter(models.SchoolClass.id == class_id).first()
    )
    class_name = (
        f"{school_class.standard} - {school_class.division}"
        if school_class
        else "Unknown"
    )
    ...
    sorted_data = sorted(
        report_data, key=lambda x: f"{x['name']} {x['surname']}".lower(), reverse=True
    )
    ...
    # Header
    elements.append(
        Paragraph(f"Monthly Attendance Report - {month_name} {year}", title_style)
    )
    ...
    # Table Styles
    col_widths = (
        [0.4 * inch, 1.8 * inch, 0.8 * inch] + [0.22 * inch] * last_day + [0.5 * inch]
    )
