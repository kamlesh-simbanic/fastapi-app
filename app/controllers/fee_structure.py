...
def create_fee_structure(
    db: Session, fee_schema: schemas.FeeStructureCreate, current_user_id: int
):
    ...
    if existing:
        raise HTTPException(
            status_code=400,
            detail="Fee structure for this class and year already exists",
        )
...
def get_fee_by_class_and_year(db: Session, class_id: int, year: int):
    fee = (
        db.query(models.FeeStructure)
        .filter(
            models.FeeStructure.class_id == class_id, models.FeeStructure.year == year
        )
        .first()
    )
    if not fee:
        raise HTTPException(
            status_code=404, detail="Fee structure not found for this class and year"
        )
    return fee
