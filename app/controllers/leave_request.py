...
from app.models import Department, LeaveRequest, LeaveStatus, Staff
from app.schemas.leave_request import LeaveRequestCreate, LeaveRequestUpdate


def create_leave_request(
    db: Session, obj_in: LeaveRequestCreate, staff_id: int, user_id: int
):
    ...
    if staff.department == Department.ADMIN:
        raise HTTPException(
            status_code=400, detail="Admin staff cannot apply for leave via this module"
        )
    ...
    print(
        f"NOTIFY (ADMIN/MGMT): New leave request from {staff.name} ({staff.department.value}) - ID: {db_obj.id}"
    )
...
def get_leave_requests(
    db: Session, staff_id: int | None = None, approver_staff: Staff | None = None
):
...
def update_leave_status(
    db: Session, leave_id: int, obj_in: LeaveRequestUpdate, approver_user_id: int
):
    ...
    if not approver_staff:
        raise HTTPException(
            status_code=403, detail="Only staff members can approve leaves"
        )
    ...
    if leave_req.staff_id == approver_staff.id:
        raise HTTPException(
            status_code=400, detail="You cannot approve your own leave request"
        )
    ...
    print(
        f"NOTIFY (STAFF): Your leave request has been {obj_in.status.value} by {approver_staff.name}"
    )
