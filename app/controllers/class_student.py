...
def add_class_student(
    db: Session, student_schema: schemas.ClassStudentCreate, current_user_id: int
):
    ...
    if existing:
        raise HTTPException(
            status_code=400,
            detail="Class mapping for this academic year already exists",
        )
...
def get_class_student(db: Session, mapping_id: int):
    mapping = (
        ...
        .filter(models.ClassStudent.id == mapping_id)
        .first()
    )
    if not mapping:
    ...
    if mapping.students:
        mapping.student_details = (
            ...
            .filter(models.Student.id.in_(mapping.students))
            .all()
        )
    else:
        mapping.student_details = []
...
def list_class_students(
    ...
    query = db.query(models.ClassStudent).options(
    ...
    items = utils.apply_pagination_sort(
        query, models.ClassStudent, skip, limit, sort_by, order
    ).all()
...
def get_students_by_class(db: Session, class_id: int):
    mapping = (
        ...
        .order_by(models.ClassStudent.id.desc())
        .first()
    )
