...
from app.models import Staff, Subject, TeacherSubject
from app.schemas import SubjectCreate, SubjectUpdate, TeacherSubjectCreate
...
def create_subject(db: Session, subject: SubjectCreate, user_id: int):
    ...
    if db_subject:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST, detail="Subject already registered"
        )

    new_subject = Subject(
        **subject.model_dump(), created_by_id=user_id, updated_by_id=user_id
    )
    db.add(new_subject)
    db.commit()
...
def assign_teacher_to_subject(
    db: Session, assignment: TeacherSubjectCreate, user_id: int
):
    ...
    new_assignment = TeacherSubject(
        **assignment.model_dump(), created_by_id=user_id, updated_by_id=user_id
    )
